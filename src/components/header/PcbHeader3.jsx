...
const Header = () => {
  ...
  useEffect(() => {
    ...
    function buildTraces() {
      ...
      for (let i = 0; i < nodes.length; i++) {
        const nearby = nodes
          .map((m, j) => ({
            j,
            d: Math.hypot(m.x - nodes[i].x, m.y - nodes[i].y),
          }))
    ...
    class Floater {
      ...
      draw() {
        const ha = Math.floor(this.alpha * 255)
          .toString(16)
          .padStart(2, "0");
        ctx.beginPath();
        ctx.arc(this.x, this.y, this.r, 0, Math.PI * 2);
        ctx.fillStyle = this.color + ha;
        ctx.fill();
        if (this.alpha > 0.4) {
          const hg = Math.floor(this.alpha * 40)
            .toString(16)
            .padStart(2, "0");
          ctx.beginPath();
          ctx.arc(this.x, this.y, this.r * 3, 0, Math.PI * 2);
          ctx.fillStyle = this.color + hg;
          ctx.fill();
        }
      }
    }
    ...
    function drawTraces() {
      for (const tr of traces) {
        ...
        ctx.lineWidth = tr.lit > 0.3 ? 1.2 : 0.7;
        ctx.stroke();
        if (tr.lit > 0.1) {
          ctx.beginPath();
          ctx.moveTo(tr.pts[0].x, tr.pts[0].y);
        ...
        for (const pt of tr.pts) {
          ...
          ctx.fillStyle = `rgba(60,140,220,${alpha * 2})`;
          ctx.fill();
        }
      }
    }
    ...
    function drawBlooms() {
      blooms = blooms.filter((b) => {
        ...
        [0.3, 0.6, 1.0].forEach((s) => {
          ctx.beginPath();
          ctx.arc(b.x, b.y, 40 * s * p, 0, Math.PI * 2);
        ...
        if (p < 0.3) {
          ctx.beginPath();
          ctx.arc(b.x, b.y, 6 * (1 - p / 0.3), 0, Math.PI * 2);
          ctx.fillStyle = `rgba(255,255,255,${0.9 * (1 - p / 0.3)})`;
          ctx.fill();
        }
        return b.age < b.life;
      });
   ...
   return (
    <>
      <header style={{ position: "relative", height: "100vh" }}>
        {/* Background logos - exact same as your original Particles version */}
        ...
        {/* Canvas - now perfectly layered and sized like Particles was */}
        <canvas
          ref={canvasRef}
          style={{ 
            position: "absolute", 
        ...
        {/* Content container - exact same structure and z-index as before */}
        <div className="container header_container" style={{ position: "relative", zIndex: 10 }}>
